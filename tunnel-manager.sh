...
# 整合版：包含完整的 Gost 配置功能
# 版本：1.9.6_Fixed
# 修复：依赖重复/MTU/接口冲突 + 严重Bug(空私钥/备份变量/
#       cron格式/菜单递归/ping6) + 新增备份还原功能(含还原全部)
# =========================================================
...
# 备份配置文件 [修复: type_dir 未定义导致备份目录创建失败]
# 参数: 类型 名称 路径 [会话名]；同一次多文件备份传同一会话名可聚合到一个目录
backup_config() {
  local config_type="$1"
  local config_name="$2"
  local config_path="$3"
  local session="${4:-$(date +%Y%m%d-%H%M%S)}"
  local backup_dir="${backup_base_dir}/${session}/${config_type}"

  mkdir -p "$backup_dir" || { echo -e "${yellow}警告: 无法创建备份目录 ${backup_dir}${plain}"; return 1; }
...
# 参数: 接口名 [会话名]；不传会话名时自动生成，同一次调用内 conf/密钥落在同一会话
backup_wireguard_config() {
  local wg_interface="$1"
  local session="${2:-$(date +%Y%m%d-%H%M%S)}"
  backup_config "wireguard" "${wg_interface}.conf" "/etc/wireguard/${wg_interface}.conf" "$session"
  [[ -f "/etc/wireguard/privatekey" ]] && backup_config "wireguard" "privatekey" "/etc/wireguard/privatekey" "$session"
  [[ -f "/etc/wireguard/${wg_interface}_publickey" ]] && backup_config "wireguard" "${wg_interface}_publickey" "/etc/wireguard/${wg_interface}_publickey" "$session"
}

# [修复] IPIP 备份路径错误：实际文件为 ipip-ddns@.timer 模板与实例
backup_ipip_config() {
  local tun_name="$1"
  local session="${2:-$(date +%Y%m%d-%H%M%S)}"
  local backup_dir="${backup_base_dir}/${session}/ipip"
  mkdir -p "$backup_dir"
  cp "/etc/systemd/system/ipip-ddns@.timer" "${backup_dir}/" 2>/dev/null
  cp "/etc/systemd/system/ipip-ddns@.service" "${backup_dir}/" 2>/dev/null
...
# 参数: [会话名]；不传时自动生成
backup_gost_config() {
  local session="${1:-$(date +%Y%m%d-%H%M%S)}"
  local backup_dir="${backup_base_dir}/${session}/gost"
  mkdir -p "$backup_dir"
...
show_ping_reminder() {
  ...
  if echo "$remote_ip" | grep -q ':' || [[ "$tunnel_type" == "ipipv6" ]]; then
    # [修复] 新版系统已移除 ping6 命令，改用 ping -6
    echo -e "${blue}  ping -6 -c 4 ${remote_ip}${plain}"
  else
...
install_wg(){
  ...
  local filename local_wg_cidr remote_wg_ip remote_wg_publickey wgport remote_endpoint localpriv
  ...
  echo ""
  echo -e "${blue}────────── 密钥说明 ──────────${plain}"
  echo -e "  本机私钥已生成: ${green}/etc/wireguard/privatekey${plain}"
  echo -e "  本机公钥已生成: ${green}/etc/wireguard/publickey${plain}"
  echo -e "  对端公钥来源:   对端机器执行 ${white}cat /etc/wireguard/publickey${plain}"
  echo -e "  ${red}注意: 填对端公钥(publickey)，切勿填私钥(privatekey)${plain}"
  echo -e "${blue}──────────────────────────────${plain}"
  echo ""
  while true; do
    echo -ne "${yellow}请粘贴对端公钥 (publickey): ${plain}"; read remote_wg_publickey_input
    remote_wg_publickey=$(echo "$remote_wg_publickey_input" | tr -d '[:space:]')
    if [[ ${#remote_wg_publickey} -ge 40 ]]; then
      break
    else
      echo -e "${red}✗ 公钥长度不足，格式错误（标准公钥约 44 字符）${plain}"
    fi
  done

  echo -ne "${yellow}监听端口 (本机 UDP 对外端口, 默认51820): ${plain}"; read wgport
  [[ -z "$wgport" ]] && wgport=51820
  echo ""
  echo -e "${blue}────────── Endpoint(对端可访问地址) ──────────${plain}"
  echo -e "  格式: ${white}公网IP或域名/可填IPIP内网:端口${plain}"
  echo -e "  示例: ${green}1.2.3.4:51820${plain} 、${green}vpn.example.com:51820${plain} 、${green}IPIP内网如 13.0.2.1:51820${plain}"
  echo -e "${blue}──────────────────────────────────────────────${plain}"
  echo ""
  echo -ne "${yellow}请输入 Endpoint: ${plain}"; read remote_endpoint
  ...
  # [修复] 原脚本使用未定义变量 localprivatekey，导致生成的配置私钥为空
...
function Install_ct() {
  check_root
  # [修复] 重装前先保存现有配置，防止 check_nor_file 删除 /etc/gost 造成转发配置丢失
  local saved_gost_dir=""
  if [[ -d /etc/gost ]] && [[ -f "$gost_conf_path" || -f "$raw_conf_path" ]]; then
    saved_gost_dir=$(mktemp -d /tmp/gost-conf-bak.XXXXXX)
    cp -a /etc/gost/. "$saved_gost_dir"/ 2>/dev/null
    echo -e "${Info} 已暂存现有 Gost 配置，安装完成后将自动恢复"
  fi
  check_nor_file
  Installation_dependency
  check_file
  ...
  # [修复] 恢复安装前保存的转发配置
  if [[ -n "$saved_gost_dir" && -d "$saved_gost_dir" ]]; then
    rm -rf /etc/gost
    mkdir -p /etc/gost
    cp -a "$saved_gost_dir"/. /etc/gost/ 2>/dev/null
    rm -rf "$saved_gost_dir"
    echo -e "${Info} 已恢复安装前的 Gost 配置"
  fi
...
function Restart_ct() {
  mkdir -p "$(dirname "$gost_conf_path")"
...
function writerawconf() {
  # [修复] 确保 /etc/gost 目录存在，避免 gost 未安装时写入失败
...
function writeconf() {
  # [修复] 原始配置不存在时跳过，避免 awk 报错
  [[ -f "$raw_conf_path" ]] || return 0
...
function show_all_conf() {
  ...
  # [修复] 原始配置不存在时给出提示而非 awk 报错
  if [[ ! -f "$raw_conf_path" ]]; then
    echo -e " ${yellow}暂无任何转发配置${plain}"
    echo -e "--------------------------------------------------------"
    return 0
  fi
...
cron_restart() {
  ...
  if [ "$numcron" == "1" ]; then
    ...
    # [修复] /etc/crontab 为5段格式且必须带用户字段，原6段Quartz格式不会生效
    if [ "$numcrontype" == "1" ]; then
      echo -e "-----------------------------------"
      read -p "每？小时重启: " cronhr
      if ! [[ "$cronhr" =~ ^[0-9]+$ ]] || [ "$cronhr" -lt 1 ]; then
        echo -e "${red}请输入大于0的整数${plain}"
        return
      fi
      ...
      if ! [[ "$cronhr" =~ ^[0-9]+$ ]] || [ "$cronhr" -gt 23 ]; then
        echo -e "${red}请输入 0-23 的整数${plain}"
        return
      fi
      echo "0 $cronhr * * * root systemctl restart gost" >>/etc/crontab
      echo -e "定时重启设置成功！"
...
# [修复] 原菜单末尾递归调用自身导致栈增长，改为 while 循环
manage_gost_services() {
  while true; do
    ...
    9)
      show_all_conf
      [[ -f "$raw_conf_path" ]] || { echo -e "${yellow}暂无配置可删除${plain}"; echo ""; echo -ne "${green}按任意键返回 Gost 服务菜单...${plain}"; read; continue; }
      read -p "请输入你要删除的配置编号：" numdelete
      if echo $numdelete | grep -q '^[0-9]\+$'; then
        total_line=$(awk 'END{print NR}' "$raw_conf_path")
        if [ "$numdelete" -ge 1 ] && [ "$numdelete" -le "$total_line" ]; then
          ...
          echo -e "${red}编号超出范围 (1-${total_line})${plain}"
        fi
      else
        echo "请输入正确数字"
      fi
...
backup_menu() {
  while true; do
    ...
    echo -e "${green}╔═══════════════════════════════════════════════════════════╗${plain}"
    echo -e "${green}║${plain}                                                           ${green}║${plain}"
    echo -e "${green}║${plain}    ${blue}配置备份还原管理${plain}                                       ${green}║${plain}"
    echo -e "${green}║${plain}                                                           ${green}║${plain}"
    echo -e "${green}╠═══════════════════════════════════════════════════════════╣${plain}"
    echo -e "${green}║${plain}  ${white}【备份】${plain}                                                 ${green}║${plain}"
    ...
    echo -e "${green}╠═══════════════════════════════════════════════════════════╣${plain}"
    echo -e "${green}║${plain}  ${white}【还原】${plain}                                                 ${green}║${plain}"
    echo -e "${green}║${plain}    ${yellow}5.${plain} 还原 WireGuard 配置                                ${green}║${plain}"
    echo -e "${green}║${plain}    ${yellow}6.${plain} 还原 IPIP 配置                                     ${green}║${plain}"
    echo -e "${green}║${plain}    ${yellow}7.${plain} 还原 Gost 配置                                     ${green}║${plain}"
    echo -e "${green}║${plain}    ${yellow}8.${plain} 还原所有配置（一次会话全部还原）                ${green}║${plain}"
    echo -e "${green}╠═══════════════════════════════════════════════════════════╣${plain}"
    echo -e "${green}║${plain}    ${white}9.${plain} 查看备份列表                                        ${green}║${plain}"
    echo -e "${green}║${plain}    ${red}0.${plain} 返回主菜单                                          ${green}║${plain}"
    echo -e "${green}╚═══════════════════════════════════════════════════════════╝${plain}"
    echo ""
    echo -e "${yellow}备份目录: ${backup_base_dir}${plain}"
    echo ""
    echo -ne "${yellow}请输入选项 [0-9]: ${plain}"
    read backup_opt
    case "$backup_opt" in
    1)
...
backup_wireguard_menu() {
  ...
  if [[ "$choice" == "$i" ]]; then
    ...
    for iface in "${wg_interfaces[@]}"; do
      backup_wireguard_config "$iface" "$session"
    done
...
# IPIP 备份菜单 [修复] grep -oP 依赖 PCRE，改用 awk 保证兼容
backup_ipip_menu() {
  clear
  echo -e "${blue}--- 备份 IPIP 配置 ---${plain}"

  # 列出所有 IPIP 接口
  local ipip_interfaces=()
  local ipip6_interfaces=()
  while IFS= read -r n; do ipip_interfaces+=("$n"); done < <(ip -o link show type ipip 2>/dev/null | awk '{print $2}' | sed 's/://g;s/@NONE//g')
  while IFS= read -r n; do ipip6_interfaces+=("$n"); done < <(ip -o link show type ip6tnl 2>/dev/null | awk '{print $2}' | sed 's/://g;s/@NONE//g')
  ...
  if [[ "$choice" == "$i" ]]; then
    ...
    session=$(date +%Y%m%d-%H%M%S)
    for iface in "${all_interfaces[@]}"; do
      backup_ipip_config "$iface" "$session"
    done
...
backup_all_configs() {
  ...
  local wg_session
  wg_session=$(date +%Y%m%d-%H%M%S)

  # 备份所有 WireGuard 配置（写入同一会话）
  for conf in /etc/wireguard/*.conf; do
    [[ -f "$conf" ]] && backup_wireguard_config "$(basename "$conf" .conf)" "$wg_session"
  done

  # 备份所有 IPIP 配置 [修复] grep -oP 改用 awk（写入同一会话）
  local ipip_all=()
  while IFS= read -r n; do ipip_all+=("$n"); done < <(ip -o link show type ipip 2>/dev/null | awk '{print $2}' | sed 's/://g;s/@NONE//g')
  while IFS= read -r n; do ipip_all+=("$n"); done < <(ip -o link show type ip6tnl 2>/dev/null | awk '{print $2}' | sed 's/://g;s/@NONE//g')
  for iface in "${ipip_all[@]}"; do
    backup_ipip_config "$iface" "$wg_session"
  done

  # 备份 Gost 配置（写入同一会话）
  if [[ -f "$gost_conf_path" ]] || [[ -d "/etc/gost" ]]; then
    backup_gost_config "$wg_session"
  fi

  echo -e "${green}所有配置已备份完成${plain}"
  echo -e "${yellow}会话目录: ${backup_base_dir}/${wg_session}${plain}"
  echo -ne "${green}按任意键返回...${plain}"
  read
}
...
# 选择包含指定类型配置的备份会话，返回会话时间戳
# 仅向 stderr 输出菜单，stdout 只输出结果，保证 $() 捕获不被污染
select_backup_session() {
  local type="$1"
  local sessions=() d s choice
  while IFS= read -r d; do
    # type=any 表示包含任意一类备份的会话，供“还原全部”使用
    if [[ "$type" == "any" ]]; then
      [[ -d "$d/wireguard" || -d "$d/ipip" || -d "$d/gost" ]] && sessions+=("$(basename "$d")")
    else
      [[ -d "$d/$type" ]] && sessions+=("$(basename "$d")")
    fi
  done < <(find "$backup_base_dir" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort -r)

  if [[ ${#sessions[@]} -eq 0 ]]; then
    if [[ "$type" == "any" ]]; then
      echo -e "${yellow}未找到任何可用备份，请先执行备份。${plain}" >&2
    else
      echo -e "${yellow}未找到包含 ${type} 配置的备份，请先执行备份。${plain}" >&2
    fi
    return 1
  fi

  echo -e "${green}可用备份会话（从新到旧）:${plain}" >&2
  local i=1
  ...
  echo "" >&2
  echo -ne "${yellow}请选择备份编号 [1-${#sessions[@]}]: ${plain}" >&2
  read choice
...
select_items_from_list() {
  ...
  echo -e "  ${i}. 全部"
  echo ""
  echo -ne "${yellow}请选择 [1-${i}]: ${plain}"
  read choice
  if [[ "$choice" == "$i" ]]; then
    SELECTED_ITEMS=("${items[@]}")
    return 0
  elif [[ "$choice" =~ ^[0-9]+$ ]] && (( choice >= 1 && choice < i )); then
    SELECTED_ITEMS=("${items[$((choice-1))]}")
    return 0
  fi
  echo -e "${red}无效选择${plain}"
  return 1
...
# 执行 WireGuard 还原（无交互提示）: $1=备份目录, 其余=要还原的接口名
do_restore_wireguard() {
  local dir="$1"; shift
  local ifc
  mkdir -p /etc/wireguard
  for ifc in "$@"; do
    ...
    # 还原私钥与公钥（若备份中包含）
    if [[ -f "$dir/privatekey" ]]; then
      cp "$dir/privatekey" /etc/wireguard/privatekey 2>/dev/null
      chmod 600 /etc/wireguard/privatekey 2>/dev/null
    fi
    [[ -f "$dir/${ifc}_publickey" ]] && cp "$dir/${ifc}_publickey" "/etc/wireguard/${ifc}_publickey" 2>/dev/null
    if cp "$dir/${ifc}.conf" "/etc/wireguard/${ifc}.conf" 2>/dev/null; then
      chmod 600 "/etc/wireguard/${ifc}.conf"
      systemctl enable "wg-quick@${ifc}" >/dev/null 2>&1
      if wg-quick up "$ifc" >/dev/null 2>&1; then
        echo -e "${green}✓ ${ifc} 已还原并启动${plain}"
      else
        echo -e "${red}${ifc} 配置已还原，但启动失败，请检查配置内容${plain}"
      fi
    else
      echo -e "${red}${ifc} 还原失败${plain}"
    fi
  done
}
...
restore_wireguard_menu() {
  clear
  echo -e "${blue}--- 还原 WireGuard 配置 ---${plain}"

  local session
  session=$(select_backup_session "wireguard") || { echo -ne "${green}按任意键返回...${plain}"; read; return; }
  local dir="${backup_base_dir}/${session}/wireguard"

  local ifaces=() f
  for f in "$dir"/*.conf; do
    [[ -f "$f" ]] && ifaces+=("$(basename "$f" .conf)")
  done
  if [[ ${#ifaces[@]} -eq 0 ]]; then
    echo -e "${yellow}该备份中没有 WireGuard 配置。${plain}"
    echo -ne "${green}按任意键返回...${plain}"; read
    return
  fi

  select_items_from_list "该备份包含以下接口" "${ifaces[@]}" || { echo -ne "${green}按任意键返回...${plain}"; read; return; }

  for ifc in "${SELECTED_ITEMS[@]}"; do
    read -p "还原 ${ifc} 将覆盖当前配置并重启接口，继续? (Y/N): " c
    [[ "$c" =~ ^[Yy]$ ]] || continue
    do_restore_wireguard "$dir" "$ifc"
  done
...
# 执行 IPIP 还原（无交互提示）: $1=备份目录, 其余=要还原的隧道名
do_restore_ipip() {
  local dir="$1"; shift
  local tun
  # 还原 keeper 脚本与 systemd 模板（若备份中包含）
  if [[ -f "$dir/$(basename "$keeper_script_path")" ]]; then
    cp "$dir/$(basename "$keeper_script_path")" "$keeper_script_path"
    chmod 755 "$keeper_script_path"
  fi
  local units_restored=0
  if [[ -f "$dir/ipip-ddns@.service" ]]; then
    cp "$dir/ipip-ddns@.service" /etc/systemd/system/ipip-ddns@.service && units_restored=1
  fi
  if [[ -f "$dir/ipip-ddns@.timer" ]]; then
    cp "$dir/ipip-ddns@.timer" /etc/systemd/system/ipip-ddns@.timer && units_restored=1
  fi
  [[ $units_restored -eq 1 ]] && systemctl daemon-reload

  install -d -m 755 "$ddns_conf_dir"
  for tun in "$@"; do
    if cp "$dir/${tun}.env" "${ddns_conf_dir}/${tun}.env" 2>/dev/null; then
      chmod 600 "${ddns_conf_dir}/${tun}.env"
      systemctl enable --now "ipip-ddns@${tun}.timer" >/dev/null 2>&1
      # 立即执行一次自愈，重建隧道接口
      systemctl start "ipip-ddns@${tun}.service" >/dev/null 2>&1
      echo -e "${green}✓ ${tun} 已还原，自愈定时器已启用并立即重建${plain}"
    else
      echo -e "${red}${tun} 还原失败${plain}"
    fi
  done
}
...
restore_ipip_menu() {
  clear
  echo -e "${blue}--- 还原 IPIP 配置 ---${plain}"

  local session
  session=$(select_backup_session "ipip") || { echo -ne "${green}按任意键返回...${plain}"; read; return; }
  local dir="${backup_base_dir}/${session}/ipip"

  local tuns=() f
  for f in "$dir"/*.env; do
    [[ -f "$f" ]] && tuns+=("$(basename "$f" .env)")
  done
  if [[ ${#tuns[@]} -eq 0 ]]; then
    echo -e "${yellow}该备份中没有 IPIP 隧道配置（.env）。${plain}"
    echo -ne "${green}按任意键返回...${plain}"; read
    return
  fi

  select_items_from_list "该备份包含以下 IPIP 隧道" "${tuns[@]}" || { echo -ne "${green}按任意键返回...${plain}"; read; return; }

  for tun in "${SELECTED_ITEMS[@]}"; do
    read -p "还原隧道 ${tun} 将覆盖当前配置并立即重建接口，继续? (Y/N): " c
    [[ "$c" =~ ^[Yy]$ ]] || continue
    do_restore_ipip "$dir" "$tun"
  done
...
# 执行 Gost 还原（无交互提示）: $1=备份目录
do_restore_gost() {
  local dir="$1"
  mkdir -p /etc/gost
  # 先整体还原目录归档，再用单个文件覆盖（更精确）
  if [[ -f "$dir/gost-config.tar.gz" ]]; then
    tar -xzf "$dir/gost-config.tar.gz" -C /etc gost 2>/dev/null && echo -e "${green}✓ 已从归档还原 /etc/gost${plain}"
  fi
  [[ -f "$dir/config.json" ]] && { cp "$dir/config.json" "$gost_conf_path" && echo -e "${green}✓ 已还原 config.json${plain}"; }
  [[ -f "$dir/rawconf" ]] && { cp "$dir/rawconf" "$raw_conf_path" && echo -e "${green}✓ 已还原 rawconf${plain}"; }
  chmod -R 777 /etc/gost 2>/dev/null

  if [[ -f /usr/lib/systemd/system/gost.service ]]; then
    systemctl restart gost >/dev/null 2>&1
    echo -e "${green}✓ Gost 配置已还原并重启服务${plain}"
  else
    echo -e "${yellow}Gost 服务未安装，配置已还原（未重启）${plain}"
  fi
}
...
restore_gost_menu() {
  clear
  echo -e "${blue}--- 还原 Gost 配置 ---${plain}"

  local session
  session=$(select_backup_session "gost") || { echo -ne "${green}按任意键返回...${plain}"; read; return; }
  local dir="${backup_base_dir}/${session}/gost"

  local has_data=0
  [[ -f "$dir/config.json" ]] && has_data=1
  [[ -f "$dir/rawconf" ]] && has_data=1
  [[ -f "$dir/gost-config.tar.gz" ]] && has_data=1
  if [[ $has_data -eq 0 ]]; then
    echo -e "${yellow}该备份中没有 Gost 配置。${plain}"
    echo -ne "${green}按任意键返回...${plain}"; read
    return
  fi

  read -p "警告: 还原将覆盖当前 Gost 配置（config.json/rawconf），继续? (Y/N): " c
  [[ "$c" =~ ^[Yy]$ ]] || return
...
# 还原所有配置 [新增] 从同一个备份会话一次性还原三类配置
restore_all_configs() {
  clear
  echo -e "${blue}--- 还原所有配置 ---${plain}"

  local session
  session=$(select_backup_session "any") || { echo -ne "${green}按任意键返回...${plain}"; read; return; }
  local base="${backup_base_dir}/${session}"

  local wg_items=() ipip_items=() f
  for f in "$base/wireguard"/*.conf; do
    [[ -f "$f" ]] && wg_items+=("$(basename "$f" .conf)")
  done
  for f in "$base/ipip"/*.env; do
    [[ -f "$f" ]] && ipip_items+=("$(basename "$f" .env)")
  done
  local has_gost=0
  if [[ -f "$base/gost/config.json" || -f "$base/gost/rawconf" || -f "$base/gost/gost-config.tar.gz" ]]; then
    has_gost=1
  fi

  if [[ ${#wg_items[@]} -eq 0 && ${#ipip_items[@]} -eq 0 && $has_gost -eq 0 ]]; then
    echo -e "${yellow}该备份会话中没有可还原的配置。${plain}"
    echo -ne "${green}按任意键返回...${plain}"; read
    return
  fi

  echo -e "${green}将从备份会话 ${session} 还原以下配置:${plain}"
  [[ ${#wg_items[@]} -gt 0 ]] && echo -e "  WireGuard: ${wg_items[*]}"
  [[ ${#ipip_items[@]} -gt 0 ]] && echo -e "  IPIP: ${ipip_items[*]}"
  [[ $has_gost -eq 1 ]] && echo -e "  Gost: config.json / rawconf / /etc/gost 归档"
  echo ""
  read -p "警告: 还原会覆盖上述全部当前配置并重启相关服务，继续? (Y/N): " c
  if [[ ! "$c" =~ ^[Yy]$ ]]; then
    echo -ne "${green}按任意键返回...${plain}"; read
    return
  fi

  [[ ${#wg_items[@]} -gt 0 ]] && do_restore_wireguard "$base/wireguard" "${wg_items[@]}"
  [[ ${#ipip_items[@]} -gt 0 ]] && do_restore_ipip "$base/ipip" "${ipip_items[@]}"
  [[ $has_gost -eq 1 ]] && do_restore_gost "$base/gost"

  echo -e "${green}全部还原流程执行完毕。${plain}"
  echo -ne "${green}按任意键返回...${plain}"
  read
}
...
${green}=== 备份与还原 ===${plain}
- 备份目录：${backup_base_dir}/<时间戳>/{wireguard,ipip,gost}
- 备份内容：
  * WireGuard: <接口>.conf + privatekey + 公钥
  * IPIP: <tun>.env + keeper 脚本 + systemd 单元
  * Gost: config.json + rawconf + /etc/gost 归档
- 还原入口：主菜单 -> 7 配置备份还原 -> 5/6/7 还原单项、8 还原整个会话的所有配置
- 选项4“备份所有配置”会把 WireGuard/IPIP/Gost 写入同一时间戳会话，配合选项8可一键全部还原。
- 还原 WireGuard/IPIP 会覆盖当前配置并重启接口，操作前会二次确认。
- 应急手动还原示例：
  cp ${backup_base_dir}/<时间戳>/wireguard/wg0.conf /etc/wireguard/
...
main_menu() {
  while true; do
    ...
    echo -e "${green}║${plain}    ${yellow}7.${plain} 配置备份还原 - WireGuard/IPIP/Gost                 ${green}║${plain}"
...
Start_Menu_WARP() {
    ...
    1)
        ...
        # [修复] git.io 已停止服务，改用 raw.githubusercontent 官方源
