...
install_ipip(){
  ...
  echo -e "${blue}  ping -c 4 ${remotevip}${plain}"
...
install_ipipv6(){
  ...
  echo -e "${yellow}测试连通性提醒：${plain}"
  echo -e "${green}建议测试对端连通性，请执行以下命令：${plain}"
  echo -e "${blue}  ping6 -c 4 ${remotevip}${plain}"
  echo -e "${yellow}如果无法 ping 通，请检查：${plain}"
...
EOF
  ...
  echo -e "${green}建议测试对端连通性，请执行以下命令：${plain}"
  if echo "$remote_wg_ip" | grep -q ':'; then
    echo -e "${blue}  ping6 -c 4 ${remote_wg_ip}${plain}"
  else
    echo -e "${blue}  ping -c 4 ${remote_wg_ip}${plain}"
  fi
  echo -e "${yellow}如果无法 ping 通，请检查：${plain}"
  echo -e "${yellow}  1. 对端 WireGuard 接口是否已正确配置和启动${plain}"
  echo -e "${yellow}  2. 对端防火墙规则是否允许 ICMP 包${plain}"
  echo -e "${yellow}  3. WireGuard 连接状态：wg show ${filename}${plain}"
